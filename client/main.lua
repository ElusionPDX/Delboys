...
local currentZone               = ''
local LastZone                  = ''
local Currenesxtion             = nil
local CurrenesxtionMsg          = ''
local CurrenesxtionData         = {}
...
function FinishDelivery()
  if IsVehicleAtesxhedToTrailer(truck) and (GetVehiclePedIsIn(GetPlayerPed(-1), false) == truck) then
    ...
    --Pay the poor fella
    TriggerServerEvent('esx_oversizetruck:pay', alldeliveries[randomdelivery].payment)
...
AddEventHandler('esx_oversizetruck:hasEnteredMarker', function(zone)
  if actualZone == 'menutrucker' then
    Currenesxtion     = 'trucker_menu'
    CurrenesxtionMsg  = 'Press ~INPUT_CONTEXT~ to go on a mission'
    CurrenesxtionData = {zone = zone}
  elseif actualZone == 'delivered' then
    Currenesxtion     = 'delivered_menu'
    CurrenesxtionMsg  = 'Press ~INPUT_CONTEXT~ for delivery'
    CurrenesxtionData = {zone = zone}
  elseif actualZone == 'abort' then
	  Currenesxtion     = 'abort_menu'
    CurrenesxtionMsg  = 'Press ~INPUT_CONTEXT~ to abort mission'
    CurrenesxtionData = {zone = zone}
  end
end)

AddEventHandler('esx_oversizetruck:hasExitedMarker', function(zone)
	Currenesxtion = nil
...
Citizen.CreateThread(function()
  while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('esx_oversizetruck:hasEnteredMarker', currentZone)
		end
		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('esx_oversizetruck:hasExitedMarker', LastZone)
		end
	end
end)
...
Citizen.CreateThread(function()
  while true do
    Citizen.Wait(0)
    if Currenesxtion ~= nil then
      SetTextComponentFormat('STRING')
      AddTextComponentString(CurrenesxtionMsg)
      DisplayHelpTextFromStringLabel(0, 0, 1, -1)
      if IsControlJustReleased(0, 38) then
        if Currenesxtion == 'trucker_menu' then
          SpawnTruck()
        elseif Currenesxtion == 'delivered_menu' then
          FinishDelivery()
		    elseif Currenesxtion == 'abort_menu' then
		      AbortDelivery()
        end
        Currenesxtion = nil
      end
